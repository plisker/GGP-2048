...
class UctTree(Tree):
	...
	def select(self):
		...
		print("select returned " + str(currentNode) + " and " + str(path))
	...
	def simulate(self):
		...
		currentNode = self
		successor,score = self.getChild(randomMove)

		# Go until game ends
		while successor != None:
			randomMove = random.choice(DIRECTIONS)
			currentNode = successor
			successor,score = successor.getChild(randomMove)

		score = currentNode.state.getScore()
	...
	def getChild(self,move):
		...
		score = newState.get_score()
		x,y = newState.get_successor(move,newState,score)
		# if x == None:
		# 	raise Exception("get_successor is returning None as successor")
		x = UctTree(x)
